...
def latent_factor_gradient_descent(M, non_zero_idx, k, val_idx, val_values, 
    ...
    for step in range(max_steps): 
        ...
        i = non_zero_matrix_temp[:, 0]
        j = non_zero_matrix_temp[:,1]
        eij = M[i,j] - np.matmul(Q, P)[i,j]
        exx = np.array(eij)
        exx.shape = ((len(i),))
        print(exx.shape)
        Q[i, :] +=  learning_rate * (2  * (P[:, j]* exx).T - 2 * reg_lambda * Q[i, :])
        print('a7a')
        P[:, j] += learning_rate * (2  * (Q[i, :].T * exx) -  2 * reg_lambda * P[:, j]) 
        print('a7a')
        Q/=number_of_users
        P/=number_of_restaurants
